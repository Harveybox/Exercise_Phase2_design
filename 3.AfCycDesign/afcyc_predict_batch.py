...
def detect_binder_chain(pdb_file: str) -> str:
    ...
    # for current RFpeptides GABARAP test, shortest chain is the macrocycle
...
def extract_ca_coords(pdb_file: str, chain_id: str):
    parser = PDBParser(QUIET=True)
    structure = parser.get_structure("x", pdb_file)
    model = next(structure.get_models())

    if chain_id not in model:
        # fallback: shortest chain as binder if requested chain is missing
        lengths = chain_lengths(pdb_file)
        chain_id = min(lengths, key=lengths.get)
    ...
    return np.array(coords, dtype=float), chain_id
...
# ----------------------------
# ColabDesign/AfCycDesign helpers
# ----------------------------
...
def set_fixed_sequence_and_predict(model, binder_seq: str):
    """
    Try the stable interfaces first.
    Different local monkey-patched builds may expose slightly different methods.
    """
    last_err = None

    # try explicit sequence -> predict(seq=...)
    ...
    # try wildtype mode if available
    if hasattr(model, "set_seq") and hasattr(model, "predict"):
        try:
...
def extract_metric_from_aux(aux: dict, names):
    """
    Different local builds stash metrics in slightly different places.
    Try a few common locations.
    """
    for k in names:
        if k in aux:
            return maybe_scalar(aux[k])

    if "log" in aux and isinstance(aux["log"], dict):
        for k in names:
            if k in aux["log"]:
                return maybe_scalar(aux["log"][k])

    return math.nan


def extract_binder_plddt(aux: dict, target_len: int, binder_len: int):
    # preferred: direct binder plddt key
    direct = extract_metric_from_aux(aux, ["plddt_binder", "binder_plddt"])
    if not math.isnan(direct):
        return direct

    # fallback: per-residue plddt over full concat, take binder tail
    plddt = None
    if "plddt" in aux:
...
def main():
    ...
    ap.add_argument("--data_dir", required=True,
                    help="directory containing AlphaFold params, e.g. params/")
    ...
    for pdb in pdbs:
        ...
        try:
            ...
            # existing complex, existing binder chain
            model.prep_inputs(
            ...
            # fixed-sequence evaluation, NOT redesign
            ...
            ref_ca, ref_chain = extract_ca_coords(str(pdb), binder_chain)
            pred_ca, pred_chain = extract_ca_coords(str(pred_pdb), binder_chain)
            binder_rmsd = calc_rmsd(ref_ca, pred_ca)
            ...
            print(f"[OK] {desc}  i_pae={i_pae:.4f}  plddt_binder={plddt_binder:.4f}  rmsd={binder_rmsd:.3f}")
